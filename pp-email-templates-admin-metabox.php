...
function pp_etemplates_template_metabox( $obj ) {
	...
	$query = "SELECT * FROM {$wpdb->prefix}options WHERE option_name LIKE 'bp-email-template-_' OR option_name LIKE 'bp-email-template-__' ORDER BY option_id ASC ";
	...
	if ( !empty( $items ) ) {
		...
		foreach( $items as $item ) {
			...
			//echo '<br>$template_exists: '  . var_dump($template_exists);
			...
			if ( ! empty ( $template_exists ) ) {
				echo '<br><br><li class="popular-category"><label class="selectit"><input value="' . $item->option_name . '" type="radio" name="bp-etemplates" ' . $checked .'>' .  $item_data['oname'] . '</label></li>';
			}
